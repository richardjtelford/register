...
#' @param course_id id number of the course.
#' Go to your course home page and the number is at the end of the url.
#' @param quiz_id The id number of the quiz. You can extract this from the output of [create_quiz()].
#' @param quiz_question_params Quiz question parameters.
#' You can make these with [collate_question_params()]
#' @returns A list with a canvas quiz.
#' @export

create_quiz_question <- function(canvas, course_id, quiz_id, quiz_question_params) {
  url <- paste0(
  ...
  response <- httr::POST(
    ...
    encode = "json")
  if (httr::status_code(response) != 200) {
    stop("Failed to create quiz question. Please check your authentication, course ID, quiz ID, and parameters. Response:", httr::status_code(response))
  }
